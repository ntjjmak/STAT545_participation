...
This is an example of dependence. That is, their is a pattern/relationship to them.
...
These are independent. X does not inform Y
...
unclass(my_lm)
names(my_lm) #pulls info from my_lm object. Then you can pull info you want. FOr example:
my_lm$coefficients
```
...
```{r}
summary(my_lm) %>% 
  names() # this calls yet another list for the function, providing Rsquare, etc.
```
...
#This gives you y values for each x value.
#Gives predicted y value for each row.

predict(my_lm, newdata = tibble(gdpPercap = c(500, 600, 700))) %>% 
  head() #newdata tells R which x values you want predictions on. You can put anything as long as those values sit within the rance of the data frame.
```
...
ggplot(gapminder, aes(gdpPercap, lifeExp)) +
    ...
    scale_x_log10()
#ggplot only draws the regression line. It will not give you any coefficients or anything numeric.
```
...
#you can specify new data here too!
glance(my_lm)
#p.value shows significance of putting x and y together
```