...
- Coercing a number to a character. (that is convert a number to a character)
...
```{r}
setName(course, c("subject", "code", "section"))
course
```
...
Subsetting a list works similarly to vectors. You can store almost anything in a list. Try subsetting the first element of `my_list`; try subsettig the first _component_ of the list. Notice the difference!
...
Note, inside this list, there is a vector of greater than one within it.
...
Coerce this into a list.
```{r}
as.list(small_df)
```

Note that there's a difference between a list of one object, and that object itself! This is different from vectors. This demonstrates that vectors don't have extra baggage around them, but lists do. Vectors are straightforward.
