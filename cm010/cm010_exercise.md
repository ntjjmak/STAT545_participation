...
### inner\_join: Join data. Retain only rows in both sets `a` to `b` by matching “x1” variable. (Only keeps the rows that match absolutely)
...
### full\_join: Join data. Retain all values, all rows of `a` to `b` by matching “x1”, (Joins all columns, even those that do not match)
...
``` r
#all rows match, so anti_join won't find anything!
```
...
#Note: cannot bind columns if row number is different
```
